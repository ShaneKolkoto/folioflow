...
// RightCard Component
const RightCard = ({ icon, title, description, color }: RightCardProps) => {
  const colorClasses = {
...
const SectionDataSharing = forwardRef<HTMLDivElement, SectionDataSharingProps>(
  ({ id, className = "" }, ref) => {
    const sharingScenarios: SharingScenario[] = useMemo(
      () => [
        {
    ...
    const neverSharedItems = useMemo(
      () => [
    ...
    const processingAgreements = useMemo(
      () => [
    ...
    return (
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className={`scroll-mt-24 bg-white rounded-2xl border border-gray-200 shadow-xl overflow-hidden ${className}`}
      >
        <div className="p-8">
          ...
          <div className="p-5 bg-linear-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {processingAgreements.map((agreement) => (
                <div key={agreement} className="flex items-start">
...
SectionDataSharing.displayName = "SectionDataSharing";

export default SectionDataSharing;